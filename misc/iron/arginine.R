...
for (i in seq_along(projects)) {
    ...
    # Violin plot
    p <- ggviolin(
        temp, x="stage", y="FPKM", fill="stage", facet.by="symbol",
    ...
    # Aesthetic settings
    p <- ggpar(
        p, xlab="Stages", ylab="log2(FPKM + 0.1)", palette="npg",
        title=projects[i], legend="right", legend.title="")
    ggsave(filename=glue("{projects[i]}.tiff"),
           plot=p, device="tiff", width=10, height=10, units="in", dpi=300)
}
