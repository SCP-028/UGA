...
initial_state =[c_AcCoA1 c_Citrate c_Isocitrate c_AlphaKG c_OXa c_AcCoA2 c_Acetate c_NAD];

[T, YY1] =ode45('odesong_201711',[0 1000],initial_state);
...
y= YY1(i,:);
       ...
       dAcCoA1 = ( (Kcat_PDHA1 * c_PDHA1 * c_Pyruvate * y(8) * max(0,c_CoA_total_M -y(1) ) ) / ( (Km_PDHA1_Pyruvate + c_Pyruvate) * (Km_PDHA1_NAD + y(8)) * (Km_PDHA1_CoA1 + max(0, c_CoA_total_M - y(1))) ) ) ...
        ...
        dCitrate = ( (Kcat_CS * c_CS * y(5) * y(1)) / ( (Km_CS_OXa + y(5)) * (Km_CS_AcCoA1 + y(1)) ) ) ...
        ...
        dIsocitrate = ( ( (Kcat_ACO2_1 * c_ACO2 * y(2)) / (Km_ACO2_Citrate + y(2)) ) - ( (Kcat_ACO2_2 * c_ACO2 * y(3)) / (Km_ACO2_Isocitrate + y(3)) ) ) ...
                - ( (Kcat_IDH2 * c_IDH2 * y(3) * y(8)) / ( (Km_IDH2_Isocitrate + y(3)) * (Km_IDH2_NAD + y(8)) ) ) ;
        % dalpha-KG = v_IDH2 - v_OGDH
        dAlphaKG = max(0,( (Kcat_IDH2 * c_IDH2 * y(3) * y(8)) / ( (Km_IDH2_Isocitrate + y(3)) * (Km_IDH2_NAD + y(8)) ) ) ...
                - ( (Kcat_OGDH * c_OGDH * y(4) * y(8)) / ( (Km_OGDH_AlphaKG + y(4)) * (Km_OGDH_NAD + y(8)) ) ) );
        % dOXa = v_OGDH + v_PC - v_CS
        dOXa =  ( (Kcat_OGDH * c_OGDH * y(4) * y(8)) / ( (Km_OGDH_AlphaKG + y(4)) * (Km_OGDH_NAD + y(8)) ) ) ...
        ...
        dAcCoA2 =  ( (Kcat_ACLY * c_ACLY * y(2) * max(0, c_CoA_total_C - y(6))) / ( (Km_ACLY_Citrate +  y(2)) * (Km_ACLY_CoA2 + max(0, c_CoA_total_C - y(6))) ) ) ...
        ...
        dAcetate = ( v_max_SLC16A3 * ( ( c_Acetate_blood / (c_Acetate_blood + k_T_Acetate) ) - ( y(7) / (y(7) + k_T_Acetate) ) ) ) ...
                + (Kcat_HDAC1 * c_HDAC1) ...
                +  (Kcat_HDAC2 * c_HDAC2) ...
                + (Kcat_HDAC3 * c_HDAC3) ...
                + ( (Kcat_ACOT12 * c_ACOT12 * y(6)) / (Km_ACOT12_AcCoA2 + y(6)) ) ...
        ...
        dNAD =  ( (v_ATP_ss / n_ATP_NAD) * (max(0, c_NAD_total - y(8)) / c_NADH_ss) ) ...
                - ( (Kcat_PDHA1 * c_PDHA1 *  c_Pyruvate * y(8)) / ( (Km_PDHA1_Pyruvate + c_Pyruvate) * (Km_PDHA1_NAD + y(8))  ) ) ...
                - ( (Kcat_IDH2 * c_IDH2 * y(3) * y(8)) / ( (Km_IDH2_Isocitrate + y(3)) * (Km_IDH2_NAD + y(8)) ) ) ...
                - ( (Kcat_OGDH *  c_OGDH * y(4) * y(8)) / ( (Km_OGDH_AlphaKG + y(4)) * (Km_OGDH_NAD + y(8)) ) ) ;
            
Rates(i,:) = [dAcCoA1, dCitrate, dIsocitrate, dAlphaKG, dOXa, dAcCoA2, dAcetate, dNAD];
     
end

%%  plot concentration   
titles = char('concentration AcCoA1', 'Citrate', 'Isocitrate', 'AlphaKG', 'OXa', 'AcCoA2', 'Acetate', 'NAD' );
for i=1:8
subplot(4,2,i);
 plot(T, YY1(:,i) );
 title(titles(i,:));
...
%%  plot Reaction Rate  
titles = char('Rate AcCoA1', 'Citrate', 'Isocitrate', 'AlphaKG', 'OXa', 'AcCoA2', 'Acetate', 'NAD' );
for i=1:8
subplot(4,2,i);
 plot(T, Rates(:,i) );
 title(titles(i,:));
