...
projects = ["TCGA-BLCA", "TCGA-BRCA", "TCGA-LUAD",
            "TCGA-UCEC", "TCGA-LUSC", "TCGA-STAD",
            "TCGA-COAD", "TCGA-READ", "TCGA-GBM"]
...
for projname in projects:
    for filepath in glob.iglob('./%s/**/*.bed' % projname, recursive=True):
        """Read all filepaths"""
        ...
        with open('%s' % filepath, 'r') as file, \
            open(os.path.join('/media/yi/Data/Bisulfite/New/%s/%s'
                 % (projname, filename)), 'w') as newfile:
