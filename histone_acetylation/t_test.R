...
result <- vector("list", 5)  # 改成gene_name长度




gene_name <- names(result) <- c("ACSS1", "ACSS2")  # Add other genes here.
...
for (i in seq_along(projects)) {
    ...
    ucp.normal <- datan[grep("(^ACSS1\\|)|(^ACSS2\\|)", rownames(datan)), , drop=F]
    ucp.tumor <- datat[grep("(^ACSS1\\|)|(^ACSS2\\|)", rownames(datat)), , drop=F]
    ...
    for (j in seq_along(gene_name)) {  # Plot for each gene in certain cancer types
        ...
        ggsave(filename=paste0("./",projects[i], "_", gene_name[j], ".png"), width = 12, height = 9, units = "in")
    }
}
